...
export default function CartItem({ item, onRemove, onChangePorsi }) {
  ...
  const total_prot = Math.round(
    (item.base_protein || item.protein) * item.porsi,
  );
  ...
  return (
    <Animated.View
      style={[styles.cartItem, nm(5), { transform: [{ scale: scaleAnim }] }]}
    >
      {}
      <Image
      ...
      <View style={styles.infoCol}>
        ...
        <Text style={styles.cartMacro}>
          {total_prot}g protein · {total_kal} kcal
        </Text>
