/**
 * JarvisTextInput.js — NutriAI Jarvis fallback: ketik alih-alih ngomong
 *
 * Popover kecil mengambang di atas box Jarvis (anchor sama persis kayak
 * JarvisVoiceOverlay), isinya cuma TextInput + tombol Kirim. Dipicu dari
 * badge "⌨" di pojok tile Jarvis (lihat QuickAccessGrid.js > JarvisTile).
 *
 * Kenapa ini penting: sebelumnya Jarvis 100% voice-only — kalau user lagi
 * di tempat berisik (warung, kantin), mic bermasalah, atau memang gak mau
 * ngomong ke HP di depan orang, gak ada jalan lain sama sekali. Sekarang
 * ada fallback yang React ke `sendText()` (fungsi yang sama yang dipakai
 * tombol "➕ Tambahkan Datanya"), jadi behavior back-end-nya udah teruji.
 *
 * Props:
 *   visible  — boolean
 *   anchor   — { x, y, width, height } | null — posisi tile Jarvis di layar
 *   sending  — boolean — true kalau lagi processing (disable input & tombol)
...
const POP_WIDTH   = 240;
const POP_HEIGHT  = 112; // estimasi — dipakai buat naruh posisi "top"
const GAP_TO_TILE = 24;  // samain sama JarvisVoiceOverlay (ada label "JARVIS"
                          // yang nongol di atas tile)
const EDGE_PAD    = 12;
...
export default function JarvisTextInput({ visible, anchor, sending, onSend, onClose }) {
  ...
  useEffect(() => {
    if (visible) {
      ...
      // Fokus input dikit didelay biar animasi modal-nya gak "kepotong" sama
      // munculnya keyboard di saat bersamaan.
  ...
  return (
    <Modal
      ...
      transparent
      animationType="none"
      onRequestClose={() => { if (!sending) onClose?.(); }}
    >
      <View style={ov.backdrop}>
        <TouchableOpacity
          style={StyleSheet.absoluteFill}
          activeOpacity={1}
          onPress={() => { if (!sending) onClose?.(); }}
        />
          ...
          <View style={ov.pop}>
            ...
            >
              ...
              <View style={ov.btnRow}>
                ...
                <TouchableOpacity
                  style={[ov.sendBtn, (sending || !text.trim()) && ov.btnDisabled]}
                  onPress={handleSend}
...
const ov = StyleSheet.create({
  ...
  sendBtn: {
    ...
    shadowColor: 'rgba(129,140,248,0.40)', shadowOffset: { width: 0, height: 4 },
