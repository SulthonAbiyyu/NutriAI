    ...
    try {
      ...
      try {
        ...
        if (data.result) {
          ...
          Alert.alert(
            "Tidak Terdeteksi",
            "AI tidak dapat menganalisis gambar ini. Coba foto yang lebih jelas.",
          );
        }
      } catch (e) {
        Alert.alert(
          ...
          e?.response?.data?.error ||
            "Analisis gagal. Pastikan GEMINI_API_KEY terkonfigurasi.",
        );
      } finally {
        setAnalyzing(false);
      }
          ...
          {analyzing ? (
            ...
            <View style={styles.aiResult}>
              ...
              <View style={styles.aiResultMacros}>
                {[
                  {
                    label: "🔥 Kalori",
                    val: `${result.estimasi_kalori} kcal`,
                    color: Colors.danger,
                  },
                  {
                    label: "💪 Protein",
                    val: `${result.estimasi_protein}g`,
                    color: Colors.primary,
                  },
                  {
                    label: "🌾 Karbo",
                    val: `${result.estimasi_karbo}g`,
                    color: Colors.warning,
                  },
                  {
                    label: "🧈 Lemak",
                    val: `${result.estimasi_lemak}g`,
                    color: Colors.blue,
                  },
                ].map((m) => (
                  <View key={m.label} style={styles.aiMacroChip}>
                    <Text style={styles.aiMacroLabel}>{m.label}</Text>
                    <Text style={[styles.aiMacroVal, { color: m.color }]}>
                      {m.val}
                    </Text>
                  </View>
                ))}
              ...
              {result.catatan ? (
                <Text style={styles.aiCatatan}>💬 {result.catatan}</Text>
              ) : null}
              <Text style={styles.aiDisclaimer}>
                ⚠️ Ini estimasi AI — nilai mungkin tidak 100% akurat
              </Text>
              <Button
            ...
            <View style={styles.aiPicker}>
              <Text style={styles.aiPickerDesc}>
                Foto makananmu dan biarkan AI mengestimasi kandungan nutrisinya
                secara otomatis!
              </Text>
              <TouchableOpacity
                style={styles.aiPickerBtn}
              ...
              <TouchableOpacity
                style={[styles.aiPickerBtn, { borderColor: Colors.blue }]}
