/**
 * JarvisCard.js — NutriAI Jarvis Voice Button
 * v2: tambah_data flow — setelah AI cari nutrisi, tampilkan form
 *     konfirmasi (gambar + gram_per_porsi) sebelum simpan ke DB.
 *
 * Props:
 *   onTalk        — () => void, trigger rekam suara / kirim teks
 *   micStatus     — 'idle' | 'recording' | 'processing'
 *   pendingFood   — object | null  { nama, kalori, protein, karbo, lemak, serat, gram_per_porsi }
 *   onConfirmFood — (formData: FormData) => Promise<void>
 *   onCancelFood  — () => void
 */
    ...
    if (active) {
      anims.forEach((anim, i) => {
        const loop = Animated.loop(
          Animated.sequence([
            Animated.timing(anim, { toValue: 1,    duration: 320 + i * 60, easing: Easing.inOut(Easing.sin), useNativeDriver: false }),
            Animated.timing(anim, { toValue: 0.25, duration: 320 + i * 60, easing: Easing.inOut(Easing.sin), useNativeDriver: false }),
          ])
        );
        loop.start();
      ...
      anims.forEach(anim => Animated.spring(anim, { toValue: 0.3, useNativeDriver: false, speed: 14 }).start());
    }
    ...
    <View style={wv.container}>
      {BAR_HEIGHTS.map((maxH, i) => (
        <Animated.View
          key={i}
          style={[wv.bar, {
            height: anims[i].interpolate({ inputRange: [0, 1], outputRange: [4, maxH] }),
...
const wv = StyleSheet.create({
  container: { flexDirection: 'row', alignItems: 'center', gap: 3, height: 36, marginBottom: 4 },
...
function PulseRing({ active, color }) {
  ...
  useEffect(() => {
    ...
    if (active) {
      scale.setValue(1); opacity.setValue(0.7);
  ...
  return (
    <Animated.View
      pointerEvents="none"
      style={{ position: 'absolute', width: 62, height: 62, borderRadius: 31, borderWidth: 2, borderColor: color, transform: [{ scale }], opacity }}
    />
  );
}

// ── NutriBadge: satu baris nilai gizi ─────────────────────
...
// ── ConfirmFoodForm — muncul saat pendingFood ada ──────────
function ConfirmFoodForm({ food, onConfirm, onCancel }) {
  const [gramPerPorsi, setGramPerPorsi] = useState(String(food.gram_per_porsi || 100));
  ...
  const pickImage = async () => {
    ...
    if (status !== 'granted') {
      Alert.alert('Izin Diperlukan', 'Izinkan akses galeri untuk memilih gambar makanan.');
      return;
    }
    ...
    if (!result.canceled && result.assets?.[0]?.uri) {
      ...
      setImageUri({ uri: asset.uri, name: `food_${Date.now()}.${ext}`, type: `image/${ext === 'jpg' ? 'jpeg' : ext}` });
    }
  };
  ...
  return (
    <Animated.View style={[cf.container, { opacity: fadeIn, transform: [{ translateY: slideY }] }]}>
      {/* Header */}
      <LinearGradient colors={[ORANGE_LT, '#FFFBEB']} style={cf.header}>
        <View style={cf.headerRow}>
          <Text style={cf.headerEmoji}>🍽️</Text>
          <View style={{ flex: 1 }}>
            <Text style={cf.headerTitle} numberOfLines={1}>{food.nama}</Text>
            <Text style={cf.headerSub}>Lengkapi data sebelum disimpan</Text>
          </View>
          <TouchableOpacity onPress={onCancel} style={cf.closeBtn} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
        ...
        <View style={cf.nutriRow}>
          <NutriBadge label="Kalori" value={food.kalori} unit="kcal" color={ORANGE} />
          <View style={cf.nutriDivider} />
          <NutriBadge label="Protein" value={food.protein} unit="g"  color={GREEN}  />
          <View style={cf.nutriDivider} />
          <NutriBadge label="Karbo"   value={food.karbo}   unit="g"  color={PURPLE} />
          <View style={cf.nutriDivider} />
          <NutriBadge label="Lemak"   value={food.lemak}   unit="g"  color={DANGER} />
        </View>
        <Text style={cf.nutriNote}>* Data nutrisi per 100g — estimasi dari AI</Text>
      </LinearGradient>
      ...
      <View style={cf.body}>
        ...
        <View style={cf.imgRow}>
          {imageUri ? (
            // Preview gambar yang dipilih — pakai Image component seperti TambahDataScreen
            <TouchableOpacity onPress={pickImage} style={cf.imgPreviewWrap}>
              <View style={cf.imgPreview}>
                <Image source={{ uri: imageUri.uri }} style={cf.imgPreviewImg} resizeMode="cover" />
              </View>
        ...
        <View style={cf.actionRow}>
          <TouchableOpacity style={cf.cancelBtn} onPress={onCancel} disabled={submitting}>
...
const cf = StyleSheet.create({
  container:      { width: '100%', borderRadius: 18, overflow: 'hidden', borderWidth: 1.5, borderColor: ORANGE_BRD, backgroundColor: WHITE, shadowColor: 'rgba(249,115,22,0.18)', shadowOffset: { width: 0, height: 6 }, shadowOpacity: 1, shadowRadius: 16, elevation: 6 },
  ...
  closeBtn:       { padding: 4, borderRadius: 8, backgroundColor: 'rgba(0,0,0,0.06)' },
  closeTxt:       { fontSize: 11, color: TXT_S, fontWeight: '700' },
  nutriRow:       { flexDirection: 'row', justifyContent: 'space-around', backgroundColor: 'rgba(255,255,255,0.7)', borderRadius: 10, paddingVertical: 8 },
  nutriDivider:   { width: 1, backgroundColor: BORDER, marginVertical: 4 },
  nutriNote:      { fontSize: 8, color: TXT_XS, textAlign: 'center', marginTop: 6, fontStyle: 'italic' },
  body:           { padding: 14, gap: 6 },
  fieldRow:       { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  fieldLabel:     { fontSize: 11, fontWeight: '700', color: TXT, marginBottom: 4, marginTop: 6 },
  fieldHint:      { fontSize: 9, color: TXT_XS, marginBottom: 6, marginTop: -2 },
  inputWrap:      { flexDirection: 'row', alignItems: 'center', backgroundColor: '#F8FAFC', borderRadius: 8, borderWidth: 1, borderColor: BORDER, paddingHorizontal: 10, height: 34 },
  input:          { fontSize: 14, fontWeight: '700', color: TXT, minWidth: 48, textAlign: 'center' },
  inputSuffix:    { fontSize: 11, color: TXT_S, fontWeight: '600', marginLeft: 4 },
  imgRow:         { marginBottom: 6 },
  imgBtnRow:      { flexDirection: 'row', gap: 8 },
  imgBtn:         { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, backgroundColor: '#F8FAFC', borderRadius: 10, borderWidth: 1, borderColor: BORDER, paddingVertical: 10 },
  imgBtnIcon:     { fontSize: 16 },
  imgBtnTxt:      { fontSize: 11, fontWeight: '600', color: TXT_S },
  imgPreviewWrap: { alignItems: 'center', gap: 4 },
  imgPreview:     { width: 72, height: 72, borderRadius: 12, overflow: 'hidden', borderWidth: 1.5, borderColor: GREEN_BRD },
  imgPreviewImg:  { width: '100%', height: '100%' },
  imgChangeHint:  { fontSize: 9, color: TXT_XS },
  actionRow:      { flexDirection: 'row', gap: 8, marginTop: 8 },
  cancelBtn:      { flex: 1, paddingVertical: 10, borderRadius: 10, borderWidth: 1.5, borderColor: BORDER, alignItems: 'center' },
  cancelTxt:      { fontSize: 12, fontWeight: '600', color: TXT_S },
  saveBtn:        { flex: 2, paddingVertical: 10, borderRadius: 10, backgroundColor: GREEN, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 6 },
...
export default function JarvisCard({
  onTalk,
  micStatus     = 'idle',
  pendingFood   = null,   // { nama, kalori, protein, karbo, lemak, serat, gram_per_porsi }
  onConfirmFood = null,   // async (formData) => void
  onCancelFood  = null,   // () => void
}) {
  ...
  // Jika ada pendingFood → tampilkan ConfirmFoodForm di atas card normal
  if (pendingFood) {
    return (
      <ConfirmFoodForm
        food={pendingFood}
  ...
  return (
    <View style={st.card}>
      ...
      >
        <Text style={[st.labelTop, { color: isRecording ? DANGER : isProcessing ? PURPLE : TXT_S }]}>
          {cfg.labelTop}
        </Text>
        <Text style={[st.labelTopBold, { color: isRecording ? DANGER : isProcessing ? PURPLE : TXT }]}>
          {cfg.labelBot}
        </Text>

        {isProcessing ? (
          <View style={{ flexDirection: 'row', gap: 6, height: 36, alignItems: 'center', marginVertical: 4 }}>
            {dotAnims.map((anim, i) => (
              <Animated.View key={i} style={{ width: 7, height: 7, borderRadius: 4, backgroundColor: PURPLE, opacity: anim }} />
            ))}
          </View>
        ) : (
          ...
          <Animated.View style={{ transform: [{ scale: pressScale }] }}>
            ...
            >
              <LinearGradient
                ...
                style={st.micBtn}
              >
                <Text style={[st.micIcon, isProcessing && { opacity: 0.7 }]}>{cfg.icon}</Text>
              </LinearGradient>
            </TouchableOpacity>
          </Animated.View>
        </View>
        ...
        <Text style={[st.hint, { color: cfg.accentColor }]}>
          {isRecording ? '● recording' : isProcessing ? '· · ·' : 'tap untuk mulai'}
        </Text>
      </LinearGradient>
    </View>
  );
...
const st = StyleSheet.create({
  ...
  labelTop:     { fontSize: 9.5, fontWeight: '500', textAlign: 'center', letterSpacing: 0.2 },
  labelTopBold: { fontSize: 12,  fontWeight: '800', textAlign: 'center', marginTop: 1, marginBottom: 2 },
  micBtn: {
  ...
  micIcon: { fontSize: 22 },
  hint:    { fontSize: 9, fontWeight: '700', letterSpacing: 0.5, marginTop: 4, textTransform: 'uppercase' },
});