...
// Inner — konsisten ~6px ke dalam dari outer (semua control point di-offset)
// endpoint: A(59,34) B(177,61) C(154,169) D(108,187) E(31,145) F(31,73)
const D_INNER = [
  'M 59,34',
...
export default function BmiCard({ bmi, label }) {
  return (
    <View style={{ width: SZ, height: H, flexShrink: 0 }}>
      ...
      <Svg width={SZ} height={H} viewBox={VB} style={{ position: 'absolute', top: 0, left: 0 }}>
        <Defs>
          ...
          {/*
            Center di (38,142) — di dalam inner path, dekat sudut E(31,145)
            tapi tidak keluar batas inner. r=185 cover sampai B(177,61).
            12 stop untuk fade realistik — 40% pertama drop dari 0.82 ke 0.20
          */}
          <RadialGradient
            id="innerGrad"
            cx="38" cy="142"
        ...
        {/* Drop shadow */}
        <Path d={D_OUTER} fill="rgba(34,197,94,0.10)" transform="translate(4,9)" />
        {/* Outer fill putih */}
        <Path d={D_OUTER} fill="url(#outerFill)" />
        <Path d={D_OUTER} fill="none" stroke="rgba(255,255,255,0.95)" strokeWidth={2.5} />
        <Path d={D_OUTER} fill="none" stroke="rgba(187,247,208,0.60)" strokeWidth={1.2} />

        {/* Inner ring gradient */}
        <Path d={D_INNER} fill="url(#innerGrad)" />
        <Path d={D_INNER} fill="none" stroke="rgba(34,197,94,0.18)" strokeWidth={0.8} />

        {/* EKG */}
        <Path
          d="M 30,105 L 55,105 L 65,82 L 80,128 L 90,100 L 105,105 L 175,105"
          fill="none" stroke="#16A34A" strokeWidth={1.5}
          opacity={0.20} strokeLinecap="round" strokeLinejoin="round"
        />
      </Svg>

      {/* Daun — pojok kanan atas, di atas konten BMI */}
      <Image
        source={DaunImg}
        style={{
          position: 'absolute',
      ...
      {/* Konten + Karakter — zIndex di atas semua */}
      <View style={{
        position: 'absolute',
        inset: 0,
        flexDirection: 'row',
        alignItems: 'flex-end',
        justifyContent: 'flex-start',   // konten lebih ke kiri
        paddingBottom: SZ * 0.06,
        paddingLeft: SZ * 0.02,          // geser ke kiri
        ...
        {/* Konten BMI — lebih kiri, lebih kecil, naik sedikit */}
        <View style={{
          alignItems: 'flex-start',
          flexShrink: 1,
          paddingBottom: SZ * 0.16,   // naik lebih tinggi dari bawah
          marginLeft: -18,
        }}>
          <Text style={{
            fontSize: SZ * 0.155,
