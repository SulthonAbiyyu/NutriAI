    ...
    try {
      ...
      if (serverData?.grouped) {
        ...
        if (allItems.length > 0) {
          await DailyRepo.upsertFromServer(
            userId,
            allItems.map((item) => ({
              id: item.id,
              ...
              kalori: item.food?.kalori || 0,
              karbo: item.food?.karbo || 0,
              lemak: item.food?.lemak || 0,
              porsi: item.food?.porsi || 1,
      ...
      return {
        grouped: localData.grouped,
        total_kalori: localData.total_kalori,
        total_protein: localData.total_protein,
        target_kalori: 0,
        target_protein: 0,
        _source: "local",
      };
